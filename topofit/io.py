...
def load_subject_data(subj, hemi, ground_truth=False, low_res=False): 
    ...
    try:
        ...
        print('two paths for read only or write access to data')
        raise
    ...
    if ground_truth:
        try:
            if os.path.isdir('whiteicosurf'):#path for read only access to data
                id = subj.split('/')[-1]
            ...
            print('two paths for read only and write access to freesurfer data respectively')
            raise        
